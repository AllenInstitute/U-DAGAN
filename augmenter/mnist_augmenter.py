...
from module.udagan import Augmenter_mnist
...
path += '/results/augmenter/mnist/'
file = 'model_bs_128_dn_16_dz_10_lambda'
load_file = path + file
...
with torch.no_grad():
    for i, (data, _) in enumerate(dataloader, 0):
        ...
        if i < 50:
            ...
            gs = gridspec.GridSpec(1, 2, width_ratios=[1, parameters['n_smp']])
            ax = plt.subplot(gs[0])
            ...
            plt.imshow(
                np.transpose(vutils.make_grid(concat_img[1:, :, :, :],
                        nrow=parameters['n_smp'], padding=2, normalize=True),
                             (1, 2, 0)), aspect='auto')
