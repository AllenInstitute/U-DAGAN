...
class augmentor(nn.Module):
    ...
    def __init__(self, input_dim, latent_dim, p_drop):
        ...
        self.fcdz1 = nn.Linear(latent_dim, latent_dim)
        self.fcdz2 = nn.Linear(latent_dim, latent_dim)
        self.fcd1 = nn.Linear(2*latent_dim, 256)
    ...
    def forward(self, x, z):
        x_low = self.encoder(x)
        z = self.relu(self.fcdz1(z))
        z = self.relu(self.fcdz2(z))
        y = torch.cat((x_low, z), dim=1)
        h1 = F.leaky_relu(self.fcd1(y), 0.2)
        h2 = F.leaky_relu(self.fcd2(h1), 0.2)
        h3 = F.leaky_relu(self.fcd3(h2), 0.2)
        return torch.tanh(self.fcd4(h3))
    ...
    if training_flag:
        ...
        for epoch in range(n_epoch):
            ...
            for batch_indx, (data, _), in enumerate(train_loader):
                ...
                z_dfake, y_fake[a] = [None] * n_aug, [None] * n_aug
                # for data in train_loader:
                data = Variable(data).cuda(device_num)
                data = data.view(-1, input_dim)
                l_real = Variable(torch.ones(batch_size)).cuda(device_num)
                ...
                for a in range(n_aug):
                    z = Variable(torch.randn(batch_size, latent_dim)).cuda(
                        device_num)
                    data_augmented[a] = a_model(data, z)
                ...
                loss_dist = (z_dfake[0] - z_dfake[1]).pow(2).mean()
            ...
            train_loss_a[epoch] = batch_train_loss_a / (batch_indx+1) #len(
            # train_loader.dataset)

            print('====> Epoch:{}, DLoss: {:.4f}, GLoss: {'
                  ':.4f}, ELoss: {:.4f}, ALoss: {:.4f}, Elapsed Time:{'
                  ':.2f}'.format(epoch, train_loss_d[epoch], train_loss_g[epoch],
                train_loss_e[epoch], train_loss_a[epoch], time.time() - t0))
    ...
    with torch.no_grad():
        for i, (data, _) in enumerate(test_loader):
            ...
            for a in range(n_aug):
                z = Variable(torch.randn(batch_size, latent_dim)).cuda(
                    device_num)
                data_augmented[a] = a_model(data.view(-1, input_dim), z)
